...
func (j *JavaHandler) Init(config ProjectConfig) error {
	...
	gradleInit := exec.Command("gradle", "init", "--type", "java-application", "--dsl", "groovy", "--test-framework", "junit-jupiter", "--project-name", config.Name, "--package", config.Name, "--no-split-project", "--java-version", "21")
	...
	fmt.Println()
	s := strings.Builder{}
	s.WriteString(ui.SuccessStyle.Render("🚀 Your Java project is ready!"))
	s.WriteString("\n\n")
