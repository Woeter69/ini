...
func (c *CSharpHandler) Init(config ProjectConfig) error {
	...
	dotnetNew := exec.Command("dotnet", "new", "console", "-n", config.Name, "-o", config.Path, "--force")
	...
	fmt.Printf("  %s C# project created\n", ui.CheckMark)
	...
	fmt.Println()
	s := strings.Builder{}
	s.WriteString(ui.SuccessStyle.Render("🚀 Your C# project is ready!"))
	s.WriteString("\n\n")
